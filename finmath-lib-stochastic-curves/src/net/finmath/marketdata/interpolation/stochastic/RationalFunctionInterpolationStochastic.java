 ...
 */
package net.finmath.marketdata.interpolation.stochastic;
...
public class RationalFunctionInterpolationStochastic  {
	...
	private class RationalFunction {
		...
		{
			RandomVariableInterface valueNumerator	= getRandomVariableForConstant(0.0);
			RandomVariableInterface valueDenominator	= getRandomVariableForConstant(0.0);
			RandomVariableInterface powerOfX	= getRandomVariableForConstant(1.0);

			for (RandomVariableInterface polynomialCoefficient : coefficientsNumerator) {
				valueNumerator = valueNumerator.addProduct(polynomialCoefficient,powerOfX);
				powerOfX = powerOfX.mult(x);
			}

			if(coefficientsDenominator == null) return valueNumerator;

			powerOfX	=  getRandomVariableForConstant(1.0);
			for (RandomVariableInterface polynomialCoefficient : coefficientsDenominator) {
				valueDenominator = valueDenominator.addProduct(polynomialCoefficient,powerOfX);
				powerOfX = powerOfX.mult(x);
			}
	...
	/**
	 * Creates a random variable from a constant with the specified factory.
	 */
	
	private RandomVariableInterface getRandomVariableForConstant(double value){
